function drawBenefitTreemapAnalysis(data) {
    ...
    if (!data || !data.benefits || !Array.isArray(data.benefits)) {
        console.error("Error: data.benefits is undefined or not an array");
        return;
    }
    ...
    const topBenefits = Array.from(benefitCounts, ([name, value]) => ({ name, value }))
        .sort((a, b) => b.value - a.value)
        .slice(0, 8);

    const treemapData = {
        name: "Benefits",
        children: topBenefits
    };

    console.log("Treemap Data (Top 8):", treemapData);

    const width = 700;
    const height = 400;
    const colorScale = d3.scaleSequential(d3.interpolateBlues)
        .domain([0, d3.max(treemapData.children, d => d.value)]);
